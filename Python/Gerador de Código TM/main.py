...
def create_file(b, file_name):
    path = 'misc/saidas/' + file_name + '.tm'

    if not os.path.exists(path):
        f = open(path, 'a') # a from create if not exists and append; b from byte reading
    else:
...
with io.open(input_file_name, "r", encoding='utf8') as file_obj:
    ...
    intermediate_code = generator.intermediate_code

    create_file(intermediate_code, input_name if input_name else grammar_name)
